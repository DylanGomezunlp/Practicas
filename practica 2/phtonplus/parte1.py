...
for i in range (3):
    if lista_vacunas[i].cantidad < 90:
        print (f'Faltan vacunas {lista_vacunas[i].get_nombre()} para {lista_vacunas[i].tipo}. {lista_vacunas[i].informacion()}')
        respuesta = input (f'Si ingresaron vacunas de {lista_vacunas[i].get_nombre()} ingrese cantidad. 0 si no ingresaron: ')
